...
Fetches Jira tickets via the REST API using a JSON config file,
applies field exclusions, and saves results to one or more JSON files.
"""
...
def load_config(config_path):
    ...
    try:
        ...
        print(f"Error: Could not decode JSON from '{config_path}': {exc}")
        return None
...
def fetch_jira_tickets(config):
    ...
    # Fixed: Validate that the access_token is not still the placeholder value
    if not access_token or access_token == "<token>":
        print("Error: 'access_token' is missing or still set to the placeholder '<token>'.")
        print("Please update jira-config.json with a valid Jira personal access token.")
        return None

    if not all([access_token, jira_base_url, jql_query]):
        print(
            "Error: Missing critical configuration: "
            "access_token, jira_base_url, or jql_query."
        )
        return None
    ...
    while True:
        payload = {
            "jql": jql_query,
            "startAt": start_at,
            # Fixed: off-by-one in log message (max_results_per_request - 1, not -1 inline)
            "maxResults": max_results_per_request,
            "fields": fields,
        }

        try:
            end_at = start_at + max_results_per_request - 1
            print(f"Requesting issues from {start_at} to {end_at}...")
            ...
            print(
                ...
                f"Total fetched so far: {len(all_issues)}."
            )
            ...
            # Fixed: response is guaranteed to exist when HTTPError is raised
            print(f"Response content: {response.content.decode(errors='replace')}")
            return None
...
def _delete_nested_key_recursive(current_item, path_segments):
    ...
    if len(path_segments) == 1:
        # Fixed: KeyError is impossible here (we checked above), but kept for safety
        current_item.pop(key_to_process, None)
    else:
...
def process_tickets_data(tickets, config):
    ...
    if not global_exclusions and not ticket_field_exclusions:
        print(
            "No exclusion rules defined in configuration. Skipping data processing."
        )
        return
    ...
    for ticket in tickets:
        ...
        for key_to_del in global_exclusions:
            # Fixed: Use pop to avoid KeyError instead of del + try/except
            ticket.pop(key_to_del, None)
        ...
        if "fields" in ticket and isinstance(ticket["fields"], dict):
            ...
            for main_field_key, sub_key_paths_to_exclude in ticket_field_exclusions.items():
                if main_field_key in ticket_fields_obj and isinstance(
                    ticket_fields_obj[main_field_key], (dict, list)
                ):
                    ...
                    for sub_key_path_str in sub_key_paths_to_exclude:
                        ...
                        _delete_nested_key_recursive(
                            target_item_for_sub_keys, path_segments
                        )
        processed_count += 1
...
def save_tickets(tickets, config):
    ...
    # Fixed: Validate export format early before doing any work
    ...
    if tickets_per_file_raw is not None:
        try:
            ...
            if tickets_per_file <= 0:
                print(
                    f"Warning: 'tickets_per_file' ({tickets_per_file_raw!r}) is not "
                    "a positive integer. Saving all tickets to a single file."
                )
                tickets_per_file = None
        except (ValueError, TypeError):
            print(
                f"Warning: 'tickets_per_file' ({tickets_per_file_raw!r}) is not a "
                "valid integer. Saving all tickets to a single file."
            )
            tickets_per_file = None
    ...
    if tickets_per_file and total_tickets > 0:
        ...
        print(
            f"Splitting {total_tickets} tickets into {num_files} file(s) "
            f"with approximately {tickets_per_file} tickets per file."
        )
        ...
        for i in range(num_files):
            ...
            try:
                ...
                print(
                    f"Successfully saved {len(ticket_chunk)} tickets to '{output_file}'"
                )
            except IOError as e:
                print(f"Error writing to file '{output_file}': {e}")
            except Exception as e:  # noqa: BLE001
                print(
                    f"An unexpected error occurred during saving chunk "
                    f"to '{output_file}': {e}"
                )
    else:
...
if __name__ == "__main__":
    if not os.path.exists(CONFIG_FILE):
        print(
            ...
            "You should create it with your Jira details."
        )
    ...
    if configuration:
        ...
        if fetched_tickets is not None:

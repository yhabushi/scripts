...
def fetch_jira_tickets(config):
    ...
    # Warn if access_token looks like a placeholder
    if access_token == "<token>":
        print("Error: access_token is still set to the placeholder value '<token>'. Please provide a real token.")
        return None
    ...
    total_issues_expected = -1  # Initialize to unknown state, updated by first API response
    ...
    while True:
        ...
        response = None  # Initialise to None so exception handlers can safely reference it
        try:
            ...
            if total_issues_expected == -1:  # If this is the first request
                total_issues_expected = data.get("total", 0)  # Get total issues matching the JQL
            ...
            # Fixed: Guard against response being None before accessing .content
            ...
            # Fixed: Guard against response being None before accessing .text
            if response is not None:
...
def process_tickets_data(tickets, config):
    ...
    for ticket in tickets:  # Iterate over each ticket
        ...
        # Fixed: Added space around += operator for PEP 8 compliance
        processed_count += 1
...
def save_tickets(tickets, config):
    ...
    tickets_per_file_config = config.get("tickets_per_file")
    tickets_per_file = None  # Initialize

    # Validate tickets_per_file from config
    if tickets_per_file_config is not None:
        try:
            tickets_per_file = int(tickets_per_file_config)
            if tickets_per_file <= 0:
                print(f"Warning: 'tickets_per_file' ({tickets_per_file_config}) is not a positive integer. Saving all tickets to a single file.")
                tickets_per_file = None  # Revert to single file behavior if invalid
        except (ValueError, TypeError):
            # Fixed: Also catch TypeError in case the value is not numeric at all
            print(f"Warning: 'tickets_per_file' ('{tickets_per_file_config}') is not a valid integer. Saving all tickets to a single file.")
...
if __name__ == "__main__":
    ...
    if not os.path.exists(CONFIG_FILE):
        ...
        example_config_content = {
            ...
            "ticket_field_exclusions": {
                ...
                "comment": [
                    ...
                    "comments.author.self", "comments.author.key", "comments.author.active",
                    "comments.author.timeZone", "comments.author.avatarUrls",
        ...
        # Fixed: Exit with a non-zero code to signal that the script did not complete successfully
        sys.exit(1)
    else:
        ...
        if configuration:
            ...
            if fetched_tickets:
                ...
                # Fixed: Exit with non-zero code on fetch failure
                sys.exit(1)
        else:
            # Fixed: Exit with non-zero code if config failed to load
            sys.exit(1)
